...
public function postSignup(Request $request)
{
    ...
    $timezone = Timezone::where('id', $defaultTimezoneId)->first();
    
    if (!$timezone) {
        $timezone = Timezone::first();
        
        if (!$timezone) {
            // Critical error: no timezones in database at all
            \Log::error("No timezones found in database during signup");
            return Redirect::back()
                ->with(['message' => 'System configuration error. Please contact support.', 'failed' => true])
                ->withInput();
        }
        
        \Log::warning("Default timezone ID {$defaultTimezoneId} doesn't exist. Using timezone ID: {$timezone->id}");
    }
    
    $account_data['timezone_id'] = $timezone->id;
